...
Create the support pool for paisley train set
    1. crop the support images and save them
    2. save the suppot dataframe 
    3. Also precomute the dataset dict for dataloader which contains assigned image_id and object_id matched with the dataframe. 
...
def get_paisley_dicts(img_dir= f'{basepath}/images/Paisley/', split='train'):
    ...
    dataset_dicts = []
    instance_id = 0
    for id, im_path in enumerate(imfns):
        record = {}
        fn = im_path
...
def main():
    ...
    for dataType in ['paisley']: #, 'train2017']:
        ...
        #coco = COCO(annFile)
        paisley_dicts = get_paisley_dicts(split='train')
        paisley_dicts_test =  get_paisley_dicts(split='test')
        with open('paisley_train_dicts.pkl', 'wb') as f:
            pickle.dump(paisley_dicts,f)
        
        with open('paisley_test_dicts.pkl', 'wb') as f:
            pickle.dump(paisley_dicts_test,f)
        
        #instance_id = 0
        for img_id, record in enumerate(paisley_dicts):
            if img_id % 100 == 0:
                print(img_id)
            ...
            frame_crop_base_path = join(set_crop_base_path, record['file_name'].split('/')[-1].split('.')[0])
            ...
            for item_id, ann in enumerate(annotations):
                ...
                support_dict['image_id'].append(image_id)
