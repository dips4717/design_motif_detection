...
class FsodRPN(nn.Module):
    ...
    def losses(
        ...
        gt_labels = torch.stack(gt_labels)  # (N, sum(Hi*Wi*Ai)) # [2, 34020]
        anchors = type(anchors[0]).cat(anchors).tensor  # Ax(4 or 5)  # [34020, 4]
        #print(anchors.shape, gt_boxes[0].shape, len(gt_boxes))
        gt_anchor_deltas = [self.box2box_transform.get_deltas(anchors, k) for k in gt_boxes]
        gt_anchor_deltas = torch.stack(gt_anchor_deltas)  # (N, sum(Hi*Wi*Ai), 4 or 5)  # [2, 34020, 4]
