...
class FsodFastRCNNOutputs(object):
    ...
    def softmax_cross_entropy_loss_nosampling(self):
        """
        Compute the softmax cross entropy loss for box classification.
        Returns:
            scalar Tensor
    ...
    def losses(self, sampled_xtloss=True):
        """
        ...
        if sampled_xtloss:
            ...
            return {
                "loss_cls": self.softmax_cross_entropy_loss_nosampling(),
                "loss_box_reg": self.smooth_l1_loss(),
            }
...
class FsodFastRCNNOutputLayers(nn.Module):
    ...
    def forward(self, x_query, x_support):
        ...
        # fc
        if self.global_relation:
            x_query_fc = self.avgpool_fc(x_query).squeeze(3).squeeze(2)   #[128, 2048]
        ...
        # correlation
        if self.local_correlation:
            x_query_cor = self.conv_cor(x_query)  # [128, 2048, 7, 7]
            support_cor = self.conv_cor(support)  # [1, 2048, 7, 7] 
            x_cor = F.relu(F.conv2d(x_query_cor, support_cor.permute(1,0,2,3), groups=2048), inplace=True).squeeze(3).squeeze(2) #[128, 2048]
            cls_score_cor = self.cls_score_cor(x_cor)

        # relation
        if self.patch_relation:
            support_relation = support.expand_as(x_query) # [128, 2048, 7, 7]
            x = torch.cat((x_query, support_relation), 1) # 
            x = F.relu(self.conv_1(x), inplace=True) # 5x5  # [128, 512, FsodFastRCNNOutputs7, 7]
            x = self.avgpool(x)                             # [128, 512, 5, 5]
            x = F.relu(self.conv_2(x), inplace=True) # 3x3  # [128, 512, 3, 3] 
            x = F.relu(self.conv_3(x), inplace=True) # 3x3  # [128, 2048, 3, 3]
            x = self.avgpool(x) # 1x1                       # [128, 2048, 1, 1]
            x = x.squeeze(3).squeeze(2)
            cls_score_pr = self.cls_score_pr(x)

        bbox_pred_all = self.bbox_pred_pr(x)                # [128, 4]   
        # final result
    ...
    def losses(self, predictions, proposals, sampled_xtloss=True):
        """
        Args:
