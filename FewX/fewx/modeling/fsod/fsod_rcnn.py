...
class FsodRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        support_images = support_images.tensor.reshape(B*N, C, H, W)  # 20x3x320x320
        support_features = self.backbone(support_images)    # res4 [40, 1024, 20, 20]
        ...
        for i in range(B): # batch
            ...
            outputs_pred_objectness_logits = [torch.cat(pos_pred_objectness_logits + neg_pred_objectness_logits, dim=0)] # [2, 34020]
            outputs_pred_anchor_deltas = [torch.cat(pos_pred_anchor_deltas + neg_pred_anchor_deltas, dim=0)]  #[2, 34020, 4]
            ...
            outputs_gt_boxes = pos_gt_boxes + neg_gt_boxes #[None] list of 2 each  [34020, 4]
            outputs_gt_labels = pos_gt_labels + neg_gt_labels  # list of 2 each  [34020]          
            if self.training:
            ...
            if self.training:
                ...
                detector_losses = self.roi_heads.box_predictor.losses(predictions, detector_proposals, sampled_xtloss=False)
